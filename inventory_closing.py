...
rebuild of one product's whole history in calendar-date order — so the
order documents were entered in never matters, only their dates.
"""
...
def recalculate(product_id):
    """Rebuild dbo.InventoryDailyClosing for one product from its first
    inbound/outbound activity date through today, and sync
    Product.StockBalance to the resulting current balance.
    ...
    all_dates = set(inbound_map) | set(outbound_map)
    if not all_dates:
    ...
    last_date = max(max(all_dates), date.today())
